...
The frontend uses Vite as the build tool and development server. Configuration is handled in `vite.config.ts`:
...
VITE_APP_TITLE=HRVSTR - Financial Analysis Platform
VITE_ENABLE_ANALYTICS=false
```

### .env.production
```env
VITE_API_BASE_URL=https://api.hrvstr.finance
...
## Component Library Configuration (Shadcn/ui)
...
3. Optimize images with proper formats and sizing
